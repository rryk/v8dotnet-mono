...
private:
    ...
    int32_t _ID; // This ID of this handle (handles are recycled and not destroyed). The ID is also used on the managed side.

    int32_t _ManagedObjectID; // The ID (index) of any associated managed object in V8.NET.  This is -1 by default, and is only update when 'GetManagedObjectID()' is called.
...
private:
    ...
    int32_t _ManagedObjectID; // If set (>=0), then this instance is a new JavaScript object, created from a template, and associated with a managed object. Default is -1.
...
public:
    ...
    ManagedAccessorInfo(ObjectTemplateProxy* objectProxy, int32_t managedObjectID, const AccessorInfo& info)
        : _ObjectProxy(objectProxy), _ManagedObjectID(managedObjectID)
    {
        Data = info.Data();
        This = info.This();
    }
